...
public class WordLadderII {
	private int N;
	private HashMap<String, ArrayList<String>> prevMap;
	...
	public ArrayList<ArrayList<String>> findLadders(String start, String end,
		...
		this.N = start.length();
		prevMap = new HashMap<String, ArrayList<String>>();
		...
		while (true) {
			for (int i = 0; i < curLevel.size(); ++i) {
				dict.remove(curLevel.get(i));
			}
			
			for (int i = 0; i < curLevel.size(); ++i) {
				getNextLevel(curLevel.get(i), dict, nextLevel);
			}
			...
			if (nextLevel.contains(end)) {
				getResult(res, path, start, end);
				return res;
			}
//			curLevel = nextLevel;
//			nextLevel = new ArrayList<String>();
			ArrayList<String> tmp = curLevel;
			curLevel.clear();
			curLevel = nextLevel;
			nextLevel = tmp;
		}
	...
	private void getResult(ArrayList<ArrayList<String>> res, ArrayList<String> path, String start, String end){
		if (start.equals(end)) {
			Collections.reverse(path);
			res.add(new ArrayList<String>(path));
			Collections.reverse(path);
		} else {
			ArrayList<String> prevs = prevMap.get(end);
			for (int i = 0; i < prevs.size(); ++i) {
				String str = prevs.get(i);
				path.add(str);
				getResult(res, path, start, str);
				path.remove(path.size()-1);
			}
		}
	}
	
	private void getNextLevel(String s, HashSet<String> dict, ArrayList<String> nextLevel) {
			...
			for (char j = 'a'; j <= 'z'; ++j) {
				...
				if (dict.contains(newWord)) {
					...
					if (prevMap.containsKey(newWord)) {
						prevMap.get(newWord).add(s);
					} else {
						ArrayList<String> prevs = new ArrayList<String>();
						prevs.add(s);
						prevMap.put(newWord, prevs);
					}
				}
			}
		}
