...
public class ConstructBinaryTreefromInorderandPreorderTraversal {
	...
	private TreeNode buildTree(int[] inorder, int instart, int inend, int[] preorder, int prestart, int preend) {
		...
		root.left = buildTree(inorder, instart, rootIndex-1, preorder, prestart+1, prestart+rootIndex-instart);
		root.right = buildTree(inorder, rootIndex+1, inend, preorder, prestart+rootIndex-instart+1, preend);
		return root;
	}
